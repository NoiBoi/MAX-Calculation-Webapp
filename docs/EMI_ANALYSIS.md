...
Electrical measurements are optional per dataset. Sample metadata contains the single authoritative thickness. A missing or invalid thickness or resistance prevents only the electrical and Simon calculations; VNA analysis remains available. Conflicting legacy thickness values are preserved and must be resolved explicitly before electrical recalculation. Simon results are additional theoretical values and never replace measured SET.
