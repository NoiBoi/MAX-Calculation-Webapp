...
Each route owns Level 3 functionality and preserves its existing scientific state architecture. Calculator and Comparison retain their dedicated command bars. EMI keeps import, analysis, visualization, and export inside the EMI route. XRD remains in development but now owns COD reference search and theoretical powder-pattern calculation through the isolated scientific service; later experimental analysis stays out of the shared shell.
