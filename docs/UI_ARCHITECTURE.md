...
Each route owns Level 3 functionality and preserves its scientific state architecture. Calculator and Comparison retain their dedicated command bars. EMI keeps import, analysis, visualization, and export inside the EMI route. XRD owns measurement import, processing, peak fitting, reference management, matching, lattice refinement, project transfer, and figure export. Its Measurements, References, Analyze, and Guide navigation remains inside the XRD route. Scientific calculations run through the isolated Python service rather than the shared application shell.
