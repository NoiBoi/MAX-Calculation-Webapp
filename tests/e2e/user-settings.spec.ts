...
test("SETTINGS-006 section and full reset preserve recipes and routes", async ({ page }) => {
  await readyWorkspace(page); await choose(page, "ti2aln"); await page.getByRole("button", { name: "Save", exact: true }).click(); const save = page.getByRole("dialog", { name: "Save recipe" }); await save.getByLabel("Recipe name").fill("Settings reset survivor"); await save.getByRole("button", { name: /Save recipe/ }).click(); await expect(page.getByText(/Saved Settings reset survivor/)).toBeVisible(); await page.getByRole("button", { name: /More actions/ }).click(); await page.getByRole("button", { name: "Manage routes" }).click(); await page.getByRole("button", { name: "Save current precursor setup as route" }).click();
  ...
  await readyWorkspace(page); await page.getByRole("button", { name: "Open", exact: true }).click(); await expect(page.getByText("Settings reset survivor", { exact: true })).toBeVisible(); await page.getByRole("button", { name: "Close library" }).click(); await page.getByRole("button", { name: /More actions/ }).click(); await page.getByRole("button", { name: "Manage routes" }).click(); await expect(page.getByText(/Ti2AlN precursor route/)).toBeVisible();
});
...
test("VERIFY-PLACEMENT-001 puts verification beside trace at the bottom and restores focus", async ({ page }) => {
  await readyWorkspace(page); await choose(page, "ti2aln"); const top = page.getByRole("toolbar", { name: "Calculator page actions" }); await expect(top.getByRole("button", { name: /Verify/ })).toHaveCount(0); await page.getByRole("button", { name: "Calculator" }).click(); await expect(page.getByRole("menuitem", { name: "Settings" })).toBeVisible(); await page.keyboard.press("Escape");
