...
test("UX-LAYOUT-001 saves and restores a bounded layout without scientific changes", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "Layouts & data" }).click();
...
test("UX-RESTORE-001 previews and restores a verified backup", async ({ page }) => {
  await page.getByRole("button", { name: "Save", exact: true }).click();
  await page.getByRole("link", { name: "Layouts & data" }).click(); await page.getByRole("button", { name: "Create verified backup" }).click();
...
test("UX-IMPORT-001/002 preserves a valid historical export and blocks tampering", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "Layouts & data" }).click(); await page.getByLabel("Choose MAX Stoich JSON file").setInputFiles({ name: "calculation.json", mimeType: "application/json", buffer: Buffer.from(json) });
...
test("UX-OFFLINE-001 and zoom: calculation, comparison, local save, and export remain usable", async ({ page, context }) => {
  ...
  await context.setOffline(false); await page.getByRole("link", { name: "Compare routes" }).click(); await expect(page.getByLabel("Scenario A scenario")).toBeVisible();
...
test("UX-ACCESS-001 completes comparison review and save controls from the keyboard", async ({ page }) => {
  await page.getByRole("link", { name: "Compare routes" }).focus(); await page.keyboard.press("Enter");
...
test("UX-ZOOM-001 keeps calculator, comparison, and data management usable at a 200%-equivalent viewport", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "Compare routes" }).click(); await expect(page.getByLabel("Scenario A scenario")).toBeVisible(); expect(await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth)).toBeLessThanOrEqual(1);
  await page.getByRole("link", { name: "Layouts & data" }).click(); await expect(page.getByRole("heading", { name: "Full local backup" })).toBeVisible(); await expect(page.getByLabel("Choose MAX Stoich JSON file")).toBeVisible(); expect(await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth)).toBeLessThanOrEqual(1);
});

test("UX-RADIUS-010 keeps atomic-radius descriptors gated without approved data", async ({ page }) => {
  await page.getByRole("button", { name: "Advanced mode" }).click();
  const radiusPanel = page.getByRole("region", { name: "Atomic radii and site descriptors" });
  await expect(radiusPanel.getByRole("heading", { name: "Atomic radii and site descriptors" })).toBeVisible();
  await expect(radiusPanel.getByText("Atomic-radius descriptors unavailable: no approved dataset is installed.", { exact: true })).toBeVisible();
  await expect(radiusPanel.getByLabel("Atomic-radius dataset")).toBeDisabled();
  await expect(radiusPanel.getByText(/Screening descriptor only/)).toBeVisible();
  await expect(page.getByText(/Mean occupied radius:/)).toHaveCount(0);
  await page.getByRole("link", { name: "Layouts & data" }).click();
  await expect(page.getByRole("heading", { name: "Atomic-radius datasets" })).toBeVisible();
  await expect(page.getByText("Installed approved datasets: 0.", { exact: false })).toBeVisible();
});
