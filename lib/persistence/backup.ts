...
async function parseBackupText(text: string): Promise<Readonly<{ backup?: MaxStoichBackup; diagnostics: ImportDiagnostic[] }>> {
  ...
  if (!value || typeof value !== "object" || (value as Record<string, unknown>).recordType !== "max-stoich-local-backup") diagnostics.push({ code: "UNKNOWN_RECORD_TYPE", severity: "error", path: "$.recordType", message: "Only MAXCalc backup files are accepted here.", blocking: true });
...
export async function previewApplicationCalculation(text: string): Promise<CalculationImportPreview> {
  ...
  inspectLimits(value, diagnostics);
  if (value.recordType !== "max-stoich-laboratory-calculation" || value.exportSchemaVersion !== "1.0.0") diagnostics.push({ code: "UNKNOWN_RECORD_TYPE", severity: "error", path: "$.recordType", message: "Only complete MAXCalc calculation exports are accepted.", blocking: true });
...
export async function previewOwnedRecord(text: string, database?: MaxStoichDatabase): Promise<OwnedRecordImportPreview> {
  ...
  if (!supported.includes(envelope.recordType) || envelope.exportSchemaVersion !== "1.0.0") diagnostics.push({ code: "UNKNOWN_RECORD_TYPE", severity: "error", path: "$.recordType", message: "The file is not a supported MAXCalc recipe, route, or comparison export.", blocking: true });
