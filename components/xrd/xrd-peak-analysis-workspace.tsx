...
function fmt(value: number | null | undefined, digits = 4): string { return value === null || value === undefined ? "—" : value.toFixed(digits).replace(/\.?0+$/, ""); }
...
export function XrdPeakAnalysisWorkspace({ database, measurementRecord, onAnalysisChange }: { readonly database: MaxStoichDatabase; readonly measurementRecord: StoredExperimentalXRDMeasurement; readonly onAnalysisChange?: (analysis: XRDPeakAnalysis | null) => void }) {
  ...
  const createPreview = async () => {
    ...
    try {
      ...
      const result = processedRepresentationSchema.parse(body); setPreview(result); setActiveProcessed(undefined); setDetection(undefined); setAnalysis(undefined); setMessage("Unsaved preview ready. Raw Stage 2 arrays remain unchanged.");
  ...
  return <section className="xrd-panel" aria-labelledby="xrd-processing-title">
    <div className="xrd-section-heading"><div><span className="xrd-step-kicker">Stage 3 · preprocessing + peak analysis</span><h2 id="xrd-processing-title">Process and fit peaks</h2><p>Every saved result is a new derived record. Automatic peaks remain researcher-correctable.</p></div></div>
    <p aria-live="polite" className="xrd-status">{message}</p>
    <div className="xrd-stage3-grid"><fieldset><legend>Processing</legend>
      <label>Baseline<select value={processingConfig.baseline.enabled ? processingConfig.baseline.algorithm : "off"} onChange={(event) => setProcessingConfig({ ...processingConfig, baseline: { ...processingConfig.baseline, enabled: event.target.value !== "off", algorithm: event.target.value === "asls" ? "asls" : "arpls" } })}><option value="off">Off</option><option value="arpls">arPLS</option><option value="asls">AsLS</option></select></label>
      <label>Smoothing<select value={processingConfig.smoothing.enabled ? "savitzky-golay" : "off"} onChange={(event) => setProcessingConfig({ ...processingConfig, smoothing: { ...processingConfig.smoothing, enabled: event.target.value !== "off" } })}><option value="off">Off</option><option value="savitzky-golay">Savitzky-Golay</option></select></label>
    ...
    {selectedGroup && activeProcessed && <div className="xrd-fit-inspection"><h3>Selected local fit</h3><FitInspectionPlot group={selectedGroup} processed={activeProcessed} /><dl className="xrd-pattern-metadata"><div><dt>Window</dt><dd>{fmt(selectedPeak?.fitWindowMinDeg)}° – {fmt(selectedPeak?.fitWindowMaxDeg)}°</dd></div><div><dt>Points</dt><dd>{selectedPeak?.pointCount}</dd></div><div><dt>Optimizer</dt><dd>{selectedPeak?.diagnostics.method ?? "—"}</dd></div><div><dt>Reduced χ²</dt><dd>{fmt(selectedPeak?.diagnostics.reducedChiSquare, 6)}</dd></div><div><dt>Components</dt><dd>{selectedGroup.candidateIds.length} peak{selectedGroup.candidateIds.length === 1 ? "" : "s"} + {analysis?.fittingConfig.localBackground} background</dd></div><div><dt>Result</dt><dd>{selectedPeak?.diagnostics.message}</dd></div></dl></div>}
